<template>
    <footer class="text-center mt-16 mb-8 text-gray-300 text-sm select-none" style="position: relative; z-index: 1;" >
      <div class="social-links mb-4 flex justify-center gap-4 items-center">
        <a :href="company.social.bluesky" target="_blank" rel="noopener noreferrer" class="social-icon">
        ...
        <a :href="company.social.twitter" target="_blank" rel="noopener noreferrer" class="social-icon">
          <!-- X/Twitter Icon -->
        ...
        <a :href="company.social.instagram" target="_blank" rel="noopener noreferrer" class="social-icon">
          <!-- Instagram Icon -->
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        ...
        <a :href="company.social.tiktok" target="_blank" rel="noopener noreferrer" class="social-icon">
          <!-- TikTok Icon -->
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        ...
        <a :href="company.social.reddit" target="_blank" rel="noopener noreferrer" class="social-icon">
      ...
      <p>© {{ currentYear }} {{ company.name }} {{ company.legalForm }} – Cosmic Run</p>
      ...
      <div v-if="showImpressum" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" @click.self="showImpressum = false">
        <div class="bg-zinc-900 border border-pink-600/50 p-6 rounded-lg max-w-2xl max-h-[80vh] overflow-y-auto">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl text-pink-500 font-bold">{{ t.impressum }}</h2>
          ...
          <div class="text-left space-y-4">
            ...
            <h3 class="text-lg text-lime-400">{{ company.name }} {{ company.legalForm }}</h3>
            <p>{{ company.address.street }}<br>{{ company.address.zip }} {{ company.address.city }}<br>{{ company.address.country }}</p>
            
            <!-- GbR Partners (All partners must be listed for a GbR) -->
            <h4 class="text-pink-500">{{ t.partners || 'Partners' }}</h4>
            <p v-for="(partner, index) in company.partners" :key="index">
              {{ partner.name }}{{ partner.title ? ', ' + partner.title : '' }}
            </p>
            
            <!-- Contact Info (Email is required, phone is optional) -->
            <h4 class="text-pink-500">{{ t.contact || 'Contact' }}</h4>
            <p>
              {{ t.email || 'Email' }}: {{ company.contact.email }}
              <template v-if="company.contact.phone">
                <br>{{ t.phone || 'Phone' }}: {{ company.contact.phone }}
              </template>
            </p>
            
            <!-- Tax Information (VAT ID if applicable) -->
            <template v-if="company.tax && company.tax.vatId">
              <h4 class="text-pink-500">{{ t.vatId || 'VAT ID' }}</h4>
              <p>{{ company.tax.vatId }}</p>
            </template>
            
            <template v-if="company.tax && company.tax.taxNumber">
              <h4 class="text-pink-500">{{ t.taxNumber || 'Tax Number' }}</h4>
              <p>{{ company.tax.taxNumber }}</p>
            </template>
          </div>
        </div>
      </div>
      ...
      <div v-if="showPrivacyPolicy" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" @click.self="showPrivacyPolicy = false">
        <div class="bg-zinc-900 border border-pink-600/50 p-6 rounded-lg max-w-2xl max-h-[80vh] overflow-y-auto">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl text-pink-500 font-bold">{{ t.privacyPolicy }}</h2>
          ...
          <div class="text-left space-y-4">
            ...
            <h4 class="text-pink-500">{{ t.dataCollection || 'Data Collection' }}</h4>
            ...
            <h4 class="text-pink-500">{{ t.googleAnalytics || 'Google Analytics' }}</h4>
            ...
            <p>{{ t.googleAnalyticsOptOut || 'You can prevent the storage of cookies by adjusting your browser settings. You can also prevent the collection of data generated by the cookie by downloading and installing the browser plugin available at:' }} <a href="https://tools.google.com/dlpage/gaoptout" class="text-pink-500 underline">https://tools.google.com/dlpage/gaoptout</a></p>
            
            <h4 class="text-pink-500">{{ t.yourRights || 'Your Rights' }}</h4>
            ...
            <p>{{ company.name }}<br>{{ t.email || 'Email' }}: {{ company.contact.privacyEmail }}</p>
          </div>
        </div>
      </div>
    </footer>
...
.social-icon {
  ...
  overflow: hidden;
  border: 1px solid rgba(236, 72, 153, 0.5); /* Pinkish border, slightly less opaque */
  margin: 0.25rem;
...
.social-icon:hover {
  ...
  background-color: rgba(236, 72, 153, 0.5); /* Brighter pink on hover */
